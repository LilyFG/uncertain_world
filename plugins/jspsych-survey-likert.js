...
jsPsych.plugins['survey-likert'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    if(trial.preamble !== null){
      html += '<div id="jspsych-survey-likert-preamble" class="jspsych-survey-likert-preamble" style="margin: 0 auto; width:80%;">'+trial.preamble+'</div>';
    }
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      for (var j = 0; j < trial.questions[i].labels.length; j++) {
        ...
        options_string += '><label class="jspsych-survey-likert-opt-label" for="'+j+"_"+i+'">' + trial.questions[i].labels[j] + '</label></li>';
      }
      options_string += '</ul>';
      html += options_string;
    }
